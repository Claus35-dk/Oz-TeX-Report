...
When planning the implementation of the server we decided to use a MSSQL database to store our data. Then we spent hours \todo{fjerne hvor længe vi brugte}{k} considering different scenarios and arranging the entities into the following data model.
\begin{figure}[H]
...
\begin{description}
\item[User] \hfill \\
Our first and possibly most important entity is the User entity. It contains information regarding each user of our system. Each user also has an optional \todo{optional country sounds weird}{Morten} country, referenced through the Country entity \todo{This entity is... Or?}{K} explained later.
...
\item[Usertype] \hfill \\
The user type defines which type the user is. \todo{Funny sentence}{Michael} The possible values in our program is:
\begin{itemize}
	\item Customer
...
Our AllowedAction entity is our way of defining permissions in the system. Whenever you need to check whether or not a user has permission to do something in our system you check the allowed action name (an example could be BAN\_UNBAN\_ANY). \todo{skift "you" ud med "we"}{k}
...
\item[MimeType] \hfill \\
MimeType is used to limit which files can be uploaded for a specific product type. When creating a product you choose which type it is of. \todo{Er det meningne at der er et punktum}{k}As it makes no sense to upload a PDF-file for a product of type `movie' we chose to limit this using the information stored here.
...
Our Transaction entity represents a finished customer purchase. \todo{Not defined to be finished}{Michael} It records the information needed by our system for a purchase. If it is a rent the expire date is also stored here.
